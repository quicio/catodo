...
export default function MediaChannel({ channelId, volume }: { channelId: string; volume: number }) {
  ...
  const chApi = (suffix: string) => `/api/channels/${channelId}${suffix}`;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const r = await fetch(chApi("/state"));
  ...
  const nav = async (cmd: string) => {
    try {
      const r = await fetch(chApi("/command"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ command: cmd }),
      });
      if (!r.ok) return;
      const st = await fetch(chApi("/state"));
  ...
  // reportar progreso al backend (resume / continuar viendo)
  ...
  if (!state || !state.current) {
    return (
      ...
      >
        {state ? `No hay contenido en ${state.name || "la biblioteca"}` : "Cargando…"}
      </div>
    );
  }

  const groups = Object.values(state.series);
  const isMovies = state.kind === "movies";
    ...
    <div style={{ position: "absolute", inset: 0, background: "#000", color: "#fff" }}>
      <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
        <video
          ref={videoRef}
          key={key}
          src={`${chApi("/stream")}?_=${key}&ts=${srcTs}`}
          autoPlay
          playsInline
          onClick={() => setControlsVisible(true)}
          onPlay={() => setIsPlaying(true)}
          onPause={() => { setIsPlaying(false); reportProgress(); }}
          onEnded={() => { reportProgress(); nav("next"); }}
          onTimeUpdate={(e) => {
          ...
          onLoadedMetadata={(e) => {
            ...
            if (current?.position_seconds) {
              try {
                v.currentTime = Math.min(current.position_seconds, v.duration - 1);
              } catch {}
            }
          }}
          style={{
      ...
      {showList && (
        ...
        >
          ...
          >
            {isMovies ? "Películas" : "Episodios"} ({state.count})
          </div>
          <div style={{ flex: 1, overflowY: "auto", padding: 8 }}>
            {isMovies
              ? groups.flatMap((s) => s.episodes).map((ep) => (
                  <button
                    key={ep.rel}
