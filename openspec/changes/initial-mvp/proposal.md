...
- **Backend (Python + FastAPI)** running as a systemd user service, exposing:
  ...
  - A `Channel` base interface implemented by two reference plugins: `Spotify` (via MPRIS/DBus) and `YouTube` (via WebView URL).
  - Default port: `8765`, bind: `127.0.0.1`.

- **Frontend (Tauri v2 + React + TypeScript)** running as a fullscreen kiosk window:
...
- Switching between Ch1 (Spotify) and Ch2 (YouTube) works via hotkey and API.
